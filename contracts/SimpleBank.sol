...
contract SimpleBank {
    ...
    mapping (address => bool)  enrolledVal;
    ...
    event LogEnrolledVal(address  accountAddress, bool enroll_val);
    ...
    // event Logmsg(Messagemsg message);
    ...
    function enrolled(address sender) public view returns (bool){
   
      // 1. enroll of the sender of this transaction
      // emit LogEnrolledVal(sender, enrolledVal[msg.sender]);
      if(enrolledVal[sender] == true) {
        return true;
      }else{
        return false;
      }
    ...
    function withdraw(uint withdrawAmount) public  returns (uint newBalance) {
      ...
      emit LogWithdrawal(msg.sender, withdrawAmount,balances[msg.sender]);
            ...
            emit LogWithdrawal(msg.sender, 0,balances[msg.sender]);
             revert();
